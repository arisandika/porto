...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-60 h-full md:bg-black/5 shrink-0 transition-all duration-300 ease-in-out overflow-hidden bg-[#111] md:backdrop-blur-lg ${
          isOpen
        ...
        <div className="flex flex-col w-64 h-full px-4 overflow-y-auto">
          ...
          <div className="flex flex-col gap-2 py-6 border-b border-[#f2f2f21a]">
            <nav className="flex flex-col gap-3">
              ...
              {siteConfig.featuredProjects.map((link) => (
                <CustomLink
                  key={link.label}
                  {...link}
                  onClick={handleLinkClick}
          ...
          <div className="flex flex-col gap-2 py-6">
            <nav className="flex flex-col gap-3">
              ...
              <Link
                ...
                onClick={handleLinkClick}
                className="inline-flex items-center gap-2 text-sm transition-colors duration-300 border-b border-dotted w-fit text-white/80 hover:text-yellow-300 border-white/20 hover:border-yellow-300/50 group"
              >
                <svg
